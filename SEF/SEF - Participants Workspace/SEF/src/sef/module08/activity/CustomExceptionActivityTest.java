...
public class CustomExceptionActivityTest {
	...
	void validateUser(String name) throws CustomExceptionActivity {
		...
		int flag = 0;
		for (int i = 0; i < 4; i++) {
			if (name.equalsIgnoreCase(validUsers[i])) {
				break;
			}
		}
		if (flag == 1) {
		...
		flag = 0;
		// 2 - Write code to check if parameter name contains a value which
		// is found in validUsers array and change flag's value accordingly

		// 3 - check if flag is zero, throw CustomExceptionActivity Exception.
		// You may also have to declare this exception in the method call using
		// throws.

		// 4 - else if flag is one, print a message
